...
def choose_cloud_converse_back(
    client_query: str, client_id, current_topic, from_social_media: bool
) -> str:
    ...
    if current_topic == Topics.GRADUATES.value:
        ...
        current_topic == Topics.UNCLASSIFIED.value
        or current_topic == Topics.SUPPORT.value
        or current_topic == Topics.ESPORTS.value
    ):
        ...
        if (
            current_topic == Topics.SUPPORT.value or "como estas" in client_query
        ):  # Interesting all greetings go to support topic.
...
def handle_admissions(client_query):
    ...
    response = assistant.message_stateless(
        assistant_id=IBM_assistant, input={"message_type": "text", "text": client_query}
    ).get_result()
...
def webhook():
    try:
        ...
        messageBody = content["entry"][0]["changes"][0]["value"]["messages"][0]["text"][
            "body"
        ]
        ...
        global AWS
        topic_for_client = AWS.dynamo_client.get_item(
            TableName="topicsForSocialMedia", Key={"clientID": {"S": f"{clientPhone}"}}
        )
        ...
        text_for_client, _ = choose_cloud_converse_back(
            messageBody, clientPhone, current_topic, from_social_media=True
        )
...
def messengerWebhook():
    try:
        ...
        global AWS
        topic_for_client = AWS.dynamo_client.get_item(
            TableName="topicsForSocialMedia", Key={"clientID": {"S": f"{PSID}"}}
        )
        ...
        print(f"current_topic is {current_topic}")
        text_for_client, _ = choose_cloud_converse_back(
            messageBody, PSID, current_topic, from_social_media=True
        )
...
def utterTextFromText():
    ...
    text_for_client, current_topic = choose_cloud_converse_back(
        client_query, client_id, current_topic, from_social_media=False
    )
...
def getTranscription():
    ...
    if best_stt_service == "Transcribe":
        ...
        transcript, confidence = AWS.transcribe_audio_file(
            "Example-job", file_key, file_key[:-4] + "json"
        )
        ...
        json_info = {
            "data": {
                "audio_response_link": f"https://buketa.s3.amazonaws.com/{file_key}"
            }
        }
        ...
        transcript = unicodedata.normalize("NFKD", weird_transcript).encode(
            "ascii", "ignore"
        )
    ...
    text_for_client, current_topic = choose_cloud_converse_back(
        transcript, client_id, current_topic, from_social_media=False
    )
    ...
    return {
        "audio_response_link": audio_response_link,
